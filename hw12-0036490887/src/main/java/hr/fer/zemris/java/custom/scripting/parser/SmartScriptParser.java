...
public class SmartScriptParser {
	...
	private EchoNode parseEcho() {
		...
		while (token.getType() != TokenType.TAG_SYMBOL_CLOSE) {
			...
			case INTEGER:
				...
				break;
			case FUNCTION:
			    String function = (String)token.getValue();
			    function = function.substring(1);
				element = new ElementFunction(function);
				break;
			case VARIABLE:
			...
			case TEXT:
				...
				element = new ElementString(text);
				break;
			case OPERATOR:
	 ...
	 * @throws SmartScriptParserException parse error
	 */
	private ForLoopNode parseFor() {
		...
		while (true) {
			...
			if (pos == 3 && token.getType() == TokenType.TAG_SYMBOL_CLOSE) {
				...
				others[pos] = new ElementString(s);
