...
public class CopyCommand implements ShellCommand {
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		Path sourceFile = Paths.get(env.getCurrentDirectory().toString(), sep[0]);
		Path destinationFile = Paths.get(env.getCurrentDirectory().toString(), sep[1]);
		
		if(!Files.exists(sourceFile)) {
			sourceFile = Paths.get(sep[0]);
		}
		if(!Files.exists(destinationFile)) {
			destinationFile = Paths.get(sep[1]);
		}
